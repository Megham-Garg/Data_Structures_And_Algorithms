...
int equalStacks(vector<int> h1, vector<int> h2, vector<int> h3) {
    int s[3];
    s[0]=accumulate(h1.begin(), h1.end(), 0);
    s[1]=accumulate(h2.begin(), h2.end(), 0);
    s[2]=accumulate(h3.begin(), h3.end(), 0);
    ...
    {
        if(s[0]==max(s[0],max(s[1],s[2]))){
            h1.erase(it[0]);
        }
        if(s[1]==max(s[0],max(s[1],s[2]))){
            h2.erase(it[1]);
        }
        if(s[2]==max(s[0],max(s[1],s[2]))){
            h3.erase(it[2]);
        }
